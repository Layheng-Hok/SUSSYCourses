<template>
  ...
  <div class="main-content">
    ...
    <div class="search-filter-section">
      ...
      >
        <template #append>
          <el-button @click="fetchCourseData">Search</el-button>
        </template>
      </el-input>
      <!-- Sorting Dropdown -->
  ...
  try {
    ...
    if (searchQuery.value) {
      courses.value = courses.value.filter(course => {
        return course.courseName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
            course.topic.toLowerCase().includes(searchQuery.value.toLowerCase());
      });
    }
  } catch (error) {
